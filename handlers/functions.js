const {
  ...
  module.exports.updateMovies2 = updateMovieWhenFollow;
...
function onCoolDown(message, command) {
  ...
  function change_status(client) {
    try {
      client.user.setActivity(`Users log their movies`, {
        type: "WATCHING",
      });
    } catch (e) {
      console.log(String(e.stack).bgRed)
...
async function updateMovies(client) {

    User.find()
        .then(arr => {
            arr.forEach(async item => {

                let feed = await parser.parseURL(`${item.profileLink}/rss/`);
                const movies = []
                const top6 = [];
                feed.items.forEach(element => {

                  let c  = element.content;
                  ...
                  const movieLink = linkify(c)
                  let link2 = movieLink.split("</p> <p>")
                  let link3 = link2[0].replace(`<p><img src="`, '').replace(`"/>`, '')
                    

                  let ui  = element.content;
                  ...
                  if(d.length == 2) {
                      finalComment = d[1].replace('</p>', "");
                  }
                  
                  else if(d.length == 3) {
                      finalComment = d[2].replace('</p>', "");
                  }
                  ...
                  if(finalComment.startsWith("Watched")) {
                    finalComment = "No review"

                  }


                  let temp_d = element.pubDate.split(" ");
                  let day = temp_d[0];
                  let temp_d2 = `${temp_d[1] + " " + temp_d[2] + " " + temp_d[3]}`

                    let name = element.title.split(" - ")[0];
                    let rating = element.title.split(" - ")[1];
                    let link = element.link;
                    let date = `${day + " " + temp_d2}`;
                    let guid = element.guid;
                    let movieCover = link3;
                    let movieReview = finalComment;
                ...
                let temp_data = await User.findOne({profileLink: item.profileLink });
                let temp_recentMovie = temp_data.recentMovie;
                

                var temp_recent = {
                    name: `${top6[0].name}`,
                    rating: `${top6[0].rating}`,
                    link: `${top6[0].link}`,
                    date: `${top6[0].date}`,
                    guid: `${top6[0].guid}`,
                    movieCover: `${top6[0].movieCover}`,
                    movieReview: `${top6[0].movieReview}`
                }

                await User.findOneAndUpdate({profileLink: item.profileLink },
                    {$set: {movies: [], name: feed.title.split(" - ")[1]}}
                    ).catch(e => {
                        console.log(e);
                     })
                ...
                await User.findOneAndUpdate({profileLink: item.profileLink },
                    {$addToSet: {movies: top6}}
                    ).catch(e => {
                        console.log(e);
                     });
                
                await User.findOneAndUpdate({profileLink: item.profileLink },
                    {$set: {recentMovie: temp_recent}}
                    ).catch(e => {
                        console.log(e);
                     });
                ...
                let data = await User.findOne({profileLink: item.profileLink});
                if(!data.recentMovie) return;
                if(data.recentMovie.guid === temp_recentMovie.guid) {
                    console.log(`no recent movie change for ${item.name}`)
                }

                else {
                  console.log(`new movie! embed sent on the channel`)
                  const movieEmbed = new MessageEmbed()
                    .setTitle(`New movie update for **${data.name}**`)
                    .setThumbnail(`${data.recentMovie.movieCover}`)
                    .addField(`Movie Name:`,`${data.recentMovie.name}`)
                    .addField(`Rating given by **${data.name}**`, `${data.recentMovie.rating}`)
                    .addField(`Date:`, `${data.recentMovie.date}`)
                    .addField(`Review :`, `${data.recentMovie.movieReview}`)
                    .setURL(`${data.recentMovie.link}`)
                    client.guilds.cache.get("626453581456998413").channels.cache.get("925051386532614184").send({ embeds: [movieEmbed] });
                }
...
async function updateMovieWhenFollow(link, paramName) {
  let feed = await parser.parseURL(`${link}/rss/`);
  const movies = []
  const top6 = [];
  feed.items.forEach(element => {
  ...
  await User.findOneAndUpdate({profileLink: link },
      {$set: {movies: []}}
      ).then(console.log(`${paramName} movie list emptied`)).catch(e => {
          console.log(e);
       })
  ...
  await User.findOneAndUpdate({profileLink: link },
      {$addToSet: {movies: top6}}
      ).then(console.log(`${paramName} movies updated`)).catch(e => {
          console.log(e);
       });
  
  await User.findOneAndUpdate({profileLink: link },
      {$set: {recentMovie: temp_recent}}
      ).then(console.log(`${paramName} recent movie updated (not clear)`)).catch(e => {
          console.log(e);
       });
}
